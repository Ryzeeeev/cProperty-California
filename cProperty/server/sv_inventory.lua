...
ESX.RegisterServerCallback("cProperty_chest:getInventoryV",function(source, cb, plate)
      ...
      for i = 1, #coffre, 1 do
        table.insert(items, {name = coffre[i].name, count = coffre[i].count, label = ESX.GetItem(coffre[i].name)})
      end
      ...
      TriggerEvent(
        ...
        function(store)
          ...
          if (blackMoney[1].amount >= count and count > 0) then
            ...
            local coffre = (store.get("coffre") or {})
            for i = 1, #coffre, 1 do
              table.insert(items, {name = coffre[i].name, count = coffre[i].count, label = ESX.GetItem(coffre[i].name), weapHash = coffre[i].compo, tint = coffre[i].tint})
            end
